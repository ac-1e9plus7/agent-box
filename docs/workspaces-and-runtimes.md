...
- During application bootstrap, the main process ensures that `.default-agent-box-workspace` exists beside the AgentBox executable. A regular directory is created automatically when missing; an existing file or symbolic link at that path is rejected. If Settings does not specify another default, this application-adjacent directory is the default workspace.
...
- Every new conversation opens the global **New conversation** dialog. Its recommended first action uses the default workspace; the user can instead switch to the current directory, up to 6 recently used directories still referenced by existing conversations, or the system directory picker. A default explicitly chosen in Settings takes precedence over the application-adjacent fallback. Recent candidates are reconstructed from retained conversation paths rather than a separate persistent directory-history list and are deduplicated by normalized full path.
