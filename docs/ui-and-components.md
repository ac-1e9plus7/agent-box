...
The browser is globally opt-in under **Settings → General → Built-in browser** and separately opt-in for Agent access in each conversation. The same Settings card exposes a home-page URL, defaulting to Google; it is opened for new sessions, user-created tabs, and the replacement tab created after the final tab closes. The panel includes a horizontally scrollable tab strip; page-created windows, switching, and closing are supported by default. The toolbar controls the active tab's address, back, forward, reload/stop, hide, and session close. On wide layouts the trusted chat pane and browser share the chat stage; on narrow layouts the browser occupies the stage. React renders only the tab strip, toolbar, status, and a bounds placeholder. The active main-process `WebContentsView` occupies that rectangle, so the renderer reports bounds through typed IPC and hides every native tab view whenever Settings or the New conversation dialog is open. Sessions are created on demand: at most three remain live application-wide and only one can be visible. Opening a fourth browser session closes the least-recently-used hidden session, so hiding a panel preserves tabs only while that session remains resident.

Manual browsing never exposes tools to the model by itself. The Composer's Browser tools switch controls the persisted `Conversation.browserToolEnabled` flag. `browser:event` carries sanitized metadata for every tab using `activeTabId` and tab `id`, plus transient download name/progress metadata; it never carries a browser absolute path. Page text and screenshot bytes reach the renderer/model only through policy-authorized Agent tool results, where they become encrypted conversation data. Settings independently enable encrypted Cookie persistence, Agent screenshots, file uploads, downloads, and loopback HTTP. Enabling uploads restores the normal user-operated system file chooser, so manual uploads may select any OS-permitted file; the separate Agent upload path accepts only approved workspace-relative files. Manual downloads use the system Downloads directory, while Agent downloads create non-overwriting workspace-relative files; both paths share the 100 MiB download cap. Closing a session discards live site storage and DOM state; when Cookie persistence is enabled, it first makes a best-effort encrypted Cookie snapshot.
