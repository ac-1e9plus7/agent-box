...
- **Anthropic:** Disabled thinking uses `{ type: "disabled" }`. The default adaptive thinking mode uses `{ type: "adaptive" }` with `output_config.effort`; AgentBox maps its `minimal` effort to Anthropic `low` and passes all other supported efforts through. Manual extended thinking uses `{ type: "enabled", budget_tokens }`; the budget is derived from reasoning effort and maximum output length, which must exceed 1,024 tokens.
...
- Native continuation applies to the Responses format. The first response opts into provider-side state with `store: true`; later model turns send `previous_response_id`, current instructions/tools, and only the new user items or function-call outputs after the saved response. Every non-stateless Responses request, including native continuation, also carries the deterministic `prompt_cache_key`; the fallback changes continuation state, not whether that cache-routing key exists. The validated opaque response handle is persisted in the encrypted conversation message so the next user turn can continue the chain.
...
Model-supplied arguments must be a JSON object and pass the tool's JSON Schema through AJV. The local read-only `agentbox_load_skill`, `agentbox_search_tools`, `agentbox_read_tool_result`, and `agentbox_read_skill_resource` helpers execute before approval evaluation; none executes a searched tool or Skill script. Other calls use the selected policy plus tool annotations or the built-in tool's risk definition. `always` prompts for every applicable call; `sensitive` automatically permits only tools explicitly declared read-only, non-destructive, and closed-world; `full-access` does not prompt. Approval can time out after five minutes or wait until the user decides or cancels.

Browser tools add parameter-aware approval and stable tab targeting. The tab-management result enumerates every `tab_id`; later calls default to the active tab but may name another. A browser-enabled Agent request acquires a conversation browser lease before the first provider turn and releases it only from Gateway final cleanup after completion, error, or cancellation. While leased, renderer-originated final-tab/session close requests are deferred and individual tab closes are blocked, preventing user interaction from invalidating an in-progress workflow; an explicit Agent close tool remains part of the model's workflow and executes immediately. A user may grant page-reading/screenshot access to one origin for the current in-memory browser session, but this never grants click, type, upload, or download permission. Navigation arguments are persisted only after sensitive URL query keys are redacted. Semantic snapshots are bounded untrusted text and screenshots are bounded JPEG tool content. Every interaction invalidates that tab's references. Responses and Anthropic carry screenshot content inside the tool result; Chat Completions receives the required text-only tool message followed by a labeled untrusted user-image message.
...
Four independent Agent token optimizations are available and are disabled by default for compatibility. Tool-result compaction keeps the complete result in renderer events and local conversation persistence, but replaces oversized model-visible text/structured replay with a deterministic head/tail preview (16,000 characters by default, configurable from 2,000 to 100,000) whose marker retains the `call_id`; bounded image media such as a browser screenshot remains typed tool content for the next provider turn, with a 2 MiB aggregate retained-image budget to keep checkpoint artifacts within their record limits. The read-only `agentbox_read_tool_result` tool can retrieve the complete serialized result in chunks. Dynamic tool exposure ranks the combined authorized built-in/MCP catalog and initially mounts at most four ranked tools by default (configurable from 1 to 16). The always-mounted read-only `agentbox_search_tools` plus enabled result/resource readers are outside that ranked limit, so the total initial definition count can be higher. Matching authorized tools can be mounted for the next model turn. Lazy Skill resources and in-run context compaction are described in their respective module documents.
...
- Provider custom headers are limited to 32 entries. Header names and values reject CR/LF, values are capped at 4,096 characters, and authentication, proxy-authentication, Cookie, Host, and Content-Length overrides are blocked.

### Global proxy

Under **Settings → General → Network proxy**, `off` connects directly. In `custom` mode, the gateway and remote MCP manager each cache an `undici.ProxyAgent`. A configuration change closes the previous dispatcher and creates a new one on the next request. Built-in browser sessions use the same setting through Electron `Session.setProxy`; the applied configuration is fingerprinted so unchanged navigation does not repeat proxy setup, while a changed setting is applied before navigation and old Chromium connections are closed. That Chromium path is distinct from `undici` dispatch.
...
- Built-in browser navigation uses one 3–30 second deadline (20 seconds by default) across proxy refresh, public-DNS validation, and `loadURL`. Stop and user-initiated close abort navigation immediately instead of waiting behind the session queue. Subresource host checks share in-flight DNS work and cancel after five seconds rather than leaving Chromium requests pending indefinitely.
