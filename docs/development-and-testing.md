...
| Agent, MCP, Skills, and code execution            | `agent-runtime`, `langgraph-agent-runtime`, `agent-continuation`, `gateway-mcp-loop`, `mcp-manager`, `mcp-schema`, `tool-retriever`, `skills-management`, `builtin-agent-tools`, `code-executor` |
