...
- The optional `agentbox_browser_*` family is exposed only when both the application-level browser and the conversation-level Browser tools switch are enabled. `agentbox_browser_tabs` lists stable tab IDs and creates, activates, or closes tabs. New-origin navigation and the first semantic page read require approval under the default policy; an origin read grant lasts only for the in-memory browser session. Clicks and text entry remain sensitive, and password, payment, one-time-code, and hidden inputs are hard-blocked even under Full Access. Screenshot, upload, and download tools appear only under their independent settings; uploads and Agent downloads are constrained to the conversation working directory.
...
Each conversation uses one non-persistent Electron session shared by its tabs. Popups become policy-checked tabs; web permissions, embedded credentials, unsupported schemes, private-network destinations, and secret form fields remain blocked. Plain HTTP is available only for loopback development URLs after an explicit setting. Cookie persistence, when enabled, snapshots accepted Chromium cookies into an encrypted per-conversation Vault profile and restores them into a new memory session; cache, storage, DOM state, tab history, references, and origin grants never persist. Manual downloads go to the system Downloads directory. Agent downloads wait for completion and create a non-overwriting workspace-relative file, with a 100 MiB limit. Uploads accept up to ten regular non-symlink workspace files, 25 MiB each and 100 MiB total.
