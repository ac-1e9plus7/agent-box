...
- **Isolated multi-tab browser:** The optional built-in browser is owned by `BrowserManager` in the main process. Every conversation receives one in-memory Chromium session and up to twelve sandboxed `WebContentsView` tabs, with no preload, Node.js, AgentBox IPC, or granted web permissions. New windows are converted into policy-checked tabs. Browser views are hidden before trusted renderer dialogs cover their reserved bounds and are destroyed when their conversation, window, or application closes. Downloads, uploads, screenshots, and cookie persistence are independent explicit settings.
...
| Browser               | `browser:ensure`, `browser:navigate`, `browser:command`, `browser:new-tab`, `browser:switch-tab`, `browser:close-tab`, `browser:set-view-state`, `browser:close`, `browser:event` | Control user-visible tabs and publish sanitized tab/download state; Agent execution, cookies, file paths, and page contents remain main-process internal |
