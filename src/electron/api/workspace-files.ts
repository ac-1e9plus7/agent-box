...
export async function readWorkspaceFile(
  ...
  try {
    ...
    if (stat.size > MAX_READ_BYTES) {
      throw new Error(
        t('The file exceeds the {value0} read limit. Use the terminal tool to process only the portions you need.', {
          value0: formatBytes(MAX_READ_BYTES),
        }),
      )
    }
    ...
    return {
      result: [
        t('[File: {value0} · Lines {value1}–{value2} of {value3}]', {
        ...
        endLine < lines.length
          ? t('[There are {value0} unread lines; continue with start_line={value1}.]', {
              value0: lines.length - endLine,
              value1: endLine + 1,
            })
          : '',
        characterTruncated
          ? t(
              '[This section contains very long text and was truncated at the tool-result size limit. Read a smaller range or process it with the terminal as needed.]',
            )
          : '',
      ]
        .filter(Boolean)
...
export async function writeWorkspaceFile(
  ...
  try {
    ...
    if (contentBytes > MAX_WRITE_BYTES) {
      throw new Error(
        t('Content exceeds the {value0} write limit. Split it and try again.', {
          value0: formatBytes(MAX_WRITE_BYTES),
        }),
      )
    }
...
export async function resolveWorkspaceFilePath(
  ...
  if (!workingDirectory || !isAbsolute(workingDirectory))
    throw new Error(t('The current conversation has no valid working directory.'))
  if (
  ...
  for (let index = 0; index < segments.length; index++) {
    ...
    try {
      ...
      if (stat.isSymbolicLink())
        throw new Error(
          t('To prevent access outside the working directory, file operations through symbolic links are not allowed.'),
        )
      if (index < segments.length - 1 && !stat.isDirectory())
        throw new Error(t('A parent path component is not a directory.'))
    } catch (error) {
...
function normalizeInteger(
  ...
  if (!Number.isInteger(value) || value < minimum || value > maximum) {
    throw new Error(
      t('{value0} must be an integer between {value1}-{value2}.', { value0: label, value1: minimum, value2: maximum }),
    )
  }
  return value
}
...
function workspaceFileError(error: unknown): string {
  ...
  if (isNodeError(error) && error.code === 'EEXIST')
    return t('The file already exists; use overwrite mode to replace it.')
  if (isNodeError(error) && (error.code === 'EACCES' || error.code === 'EPERM'))
    return t('You do not have permission to read or write this file.')
