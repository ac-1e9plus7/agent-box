...
import { ContextWindowError, prepareMessagesForContext, resolveContextManagementMode } from './context-window'
...
export class ChatGateway {
  ...
  async stream(requestId: string, request: ChatRequest, emit: StreamEmitter): Promise<void> {
    ...
    const resetStallTimer = () => {
      ...
      stallTimer = setTimeout(
        () =>
          controller.abort(
            new GatewayError(
              t('The request stream stalled for more than 120 seconds and was stopped.'),
              'request_timeout',
            ),
          ),
        120_000,
    ...
    try {
      ...
      const runtimeResult = await runAgentRuntime<StreamConsumptionResult>({
        ...
        executeTools: async ({ messages: currentTurnMessages, modelResult: streamResult, turn }) => {
          ...
          agentTrace.push(
            ...streamResult.anthropicThinkingBlocks.map((block) => ({
          ...
          for (const rawCall of streamResult.toolCalls) {
            ...
            try {
              ...
              argumentError = t('Tool parameters are not valid JSON: {value0}', {
                value0: error instanceof Error ? error.message : String(error),
              })
            }

            const validation = toolDef && !argumentError ? validateToolArguments(toolDef, parsedValue) : undefined
            ...
            if (failure || !toolDef) {
              ...
              emit({
                ...
                callId: rawCall.id,
                toolName: displayName,
                result: failure || t('The tool is unavailable.'),
                isError: true,
                turn,
              })
              toolExecutions.push({
            ...
            if (isCodeRunner) {
              ...
              const timeoutSeconds =
                typeof parsedArgs.timeout_seconds === 'number' ? parsedArgs.timeout_seconds : undefined
                ...
                : await writeWorkspaceFile(
                    workingDirectory,
                    {
                      ...
                      createParentDirectories:
                        typeof parsedArgs.create_parent_directories === 'boolean'
  ...
  resolveToolApproval(requestId: string, callId: string, approved: boolean): void {
    ...
    if (!pending)
      throw new GatewayError(
        t('This tool approval request does not exist or has already ended.'),
  ...
  private async discoverModelsFromProvider(provider: StoredProvider): Promise<RemoteModel[]> {
      ...
      response = await fetch(
        ...
        this.withProxy({
          headers: buildProviderHeaders(
            provider,
            provider.kind === 'cliproxy' ? 'openai-chat-completions' : provider.apiFormat,
            false,
          ),
          redirect: 'error',
    ...
    if (responseBody.truncated) {
      throw new GatewayError(
        t('The model list returned by the provider exceeds the 32 MiB limit.'),
        'response_too_large',
      )
    }
    let payload: unknown
...
function buildAgentSystemPrompt(
  ...
  parts.push(
    t('[Agent mode is enabled]') +
      ...
      t(
        '5. Skill scripts are reference code by default. Never claim they were executed unless an explicit restricted execution tool ran them.',
      ),
  )
  ...
  if (externalTools.length > 0) {
    parts.push(
      t('=== Active MCP Tools ===\n') +
        externalTools
          .map((tool) =>
            t('- `{value0}` (display name: {value1}, source: {value2})', {
              value0: tool.modelName || tool.name,
              value1: tool.name,
              value2: tool.serverName,
            }),
          )
          .join('\n'),
    )
  ...
  if (terminalRunnerAvailable) {
    parts.push(
      t(
        ...
        { value0: TERMINAL_RUNNER_MODEL_NAME },
      ),
    )
  }
      ...
      t('=== Available Skill Catalog (routing only) ===\n') +
        ...
        (skillLoaderAvailable
          ? t(
              ...
              { value0: SKILL_LOADER_MODEL_NAME },
            )
          : ''),
    )
  }
          ...
          .map((skill, index) => {
            ...
            const skillSections: string[] = [
              t(
                '[Skill {value0}: {value1}] (ID: {value2}, version: {value3})\nDescription: {value4}\n\n## Operating Guidelines and Core Instructions\n{value5}',
                {
                  ...
                  value1: skill.name,
                  value2: skill.id,
                  value3: skill.version ?? '1.0.0',
                  value4: skill.description,
                  value5: entryDoc.trim(),
                },
              ),
            ]
            ...
            if (pythonScripts.length > 0) {
              const pySection = pythonScripts
                .map((s) =>
                  t('### Python 3 script: `{value0}`\n```python\n{value1}\n```', {
                    value0: s.path,
                    value1: s.content.trim(),
                  }),
                )
                .join('\n\n')
              skillSections.push(
                t('## Bundled Python 3 Reference Scripts (not run automatically):\n{value0}', { value0: pySection }),
              )
            }

            if (shellScripts.length > 0) {
              const shSection = shellScripts
                .map((s) =>
                  t('### Shell script: `{value0}`\n```bash\n{value1}\n```', {
                    value0: s.path,
                    value1: s.content.trim(),
                  }),
                )
                .join('\n\n')
              skillSections.push(
                t('## Bundled Shell Reference Scripts (not run automatically):\n{value0}', { value0: shSection }),
              )
            }

            if (otherDocs.length > 0) {
              const docSection = otherDocs
                .map((d) =>
                  t('### Reference document: `{value0}`\n{value1}', { value0: d.path, value1: d.content.trim() }),
                )
                .join('\n\n')
...
function addConfiguredSystemPrompt(messages: Message[], systemPrompt: string): Message[] {
  ...
  if (messages.some((message) => message.role === 'system' && message.content.trim() === trimmedPrompt)) {
...
function validateChatRequest(request: ChatRequest): Message[] {
  ...
  if (
    ...
    (typeof request.workingDirectory !== 'string' ||
      ...
      /[\r\n\0]/.test(request.workingDirectory) ||
  ...
  if (request.webSearchMode !== undefined && !['off', 'auto', 'native'].includes(String(request.webSearchMode))) {
    throw new GatewayError(t('Invalid web search mode.'), 'invalid_request')
  }
  if (request.allowContextTrimming !== undefined && typeof request.allowContextTrimming !== 'boolean') {
    throw new GatewayError(t('Invalid context-trimming option.'), 'invalid_request')
  }
  if (
    request.reasoningEffort !== undefined &&
    !['minimal', 'low', 'medium', 'high', 'xhigh', 'max'].includes(String(request.reasoningEffort))
  ) {
    throw new GatewayError(t('Invalid reasoning effort.'), 'invalid_request')
  }
  if (
    request.maxOutputTokens !== undefined &&
    (!Number.isInteger(request.maxOutputTokens) || request.maxOutputTokens <= 0 || request.maxOutputTokens > 10_000_000)
  ) {
...
function resolveResumeFromMessageId(request: ChatRequest, messages: Message[], agentMode: boolean): string | undefined {
  ...
  if (!agentMode)
    throw new GatewayError(t('Agent mode must be turned on to continue Agent execution.'), 'invalid_request')
  ...
  if (
    ...
    throw new GatewayError(
      t('The Agent resume checkpoint must be the final assistant message before the current user instruction.'),
      'invalid_request',
    )
  }
...
function sanitizeRequestAttachments(value: unknown): Message['attachments'] {
  ...
  if (!Array.isArray(value) || value.length > 20)
    throw new GatewayError(t('The attachment list is invalid.'), 'invalid_request')
...
function sanitizeRequestToolExecutions(value: unknown): ToolCallExecution[] | undefined {
  ...
  return value.map((execution) => {
    ...
    return {
      ...
      approvalReason:
        typeof execution.approvalReason === 'string' ? execution.approvalReason.slice(0, 2_000) : undefined,
      status,
    }
  })
}
...
function cloneJsonRecord(value: Record<string, unknown>, maxCharacters: number): Record<string, unknown> {
  ...
  if (serialized.length > maxCharacters)
    throw new GatewayError(t('Agent structured data exceeds limit.'), 'invalid_request')
