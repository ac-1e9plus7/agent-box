...
function isSkillFile(value: unknown): value is SkillFile {
  ...
  return (
    ...
    (value.kind === 'markdown' || value.kind === 'python' || value.kind === 'shell' || value.kind === 'other')
  )
}

function parseSkillFiles(value: unknown): SkillFile[] | undefined {
  if (!Array.isArray(value)) return undefined
  const candidates: unknown[] = value
  return candidates.every(isSkillFile) ? candidates : undefined
}
...
export function SkillsTab({
    ...
    try {
      ...
      setSkillImportError(
        err instanceof Error
          ? err.message
          : t('Could not parse or import the skill archive. Check the archive contents.'),
      )
    }
  }
  ...
  const handleImportSkillText = async (): Promise<void> => {
    ...
    try {
      const parsed: unknown = JSON.parse(skillImportText.trim())
      const items: unknown[] = Array.isArray(parsed) ? parsed : [parsed]
      for (const item of items) {
        if (!isRecord(item)) throw new Error(t('Invalid skill configuration format'))
      ...
      setSkillImportError(
        err instanceof Error ? err.message : t('Could not parse or import the configuration. Check its format.'),
      )
    }
  }
    ...
    <div className="settings-section-content skills-settings">
      ...
      {skillActionError && (
        <div className="settings-error-banner" role="alert">
          ...
          <button className="icon-button" onClick={() => setSkillActionError('')}>
            <Icon name="close" size={13} />
          </button>
        </div>
      )}
      ...
      <div className="skills-grid">
        {filteredSkills.length === 0 ? (
          <div className="skills-empty">
            ...
            <small>
              {t(
                'Select “New skill” or “Import skill” above to add capabilities; `.zip` skill archives are supported.',
              )}
            </small>
          </div>
        ) : (
                ...
                <div className="skill-file-tags">
                  {mdCount > 0 && (
                    <span
                      ...
                      title={t('{value0} Markdown documents', { value0: mdCount })}
                    >
                  ...
                  {pyCount > 0 && (
                    <span
                      className="skill-tag skill-tag-py"
                      title={t('{value0} Python 3 scripts', { value0: pyCount })}
                    >
          ...
          <div className="skill-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="skill-modal-body">
              ...
              <label className="skill-form-field">
                ...
                <textarea
                  placeholder={t(
                    'Define the execution guidelines, reasoning guidance, and output format the Agent should follow when this Skill is active…',
                  )}
                  rows={8}
            ...
            <footer className="skill-modal-footer">
              <button className="secondary-button" onClick={() => setEditingSkill(null)}>
                {t('Cancel')}
              </button>
              <button
        ...
        <div className="skill-modal-backdrop" onClick={() => setInstallingSkill(false)}>
          <div className="skill-modal" onClick={(e) => e.stopPropagation()}>
            <header className="skill-modal-header">
              ...
              <button className="icon-button" onClick={() => setInstallingSkill(false)}>
                <Icon name="close" size={16} />
              </button>
            </header>
            <div className="skill-modal-body">
              <p className="skill-modal-hint">
                {t(
                  'Recommended: import a ZIP skill archive (.zip) containing SKILL.md, any Python 3 or shell scripts, and reference documents.',
                )}
              </p>
            ...
            <footer className="skill-modal-footer">
              <button className="secondary-button" onClick={() => setInstallingSkill(false)}>
                {t('Cancel')}
              </button>
              <button
                className="primary-button"
