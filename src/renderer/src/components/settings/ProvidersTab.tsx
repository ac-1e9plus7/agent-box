...
export function ProvidersTab({
  ...
  const updateProvider = (patch: Partial<ProviderConfig>): void => {
    setProviderDrafts((current) =>
      current.map((provider) => (provider.id === selectedProviderId ? { ...provider, ...patch } : provider)),
    )
  }
  ...
  const toggleClearApiKey = (providerId: string): void => {
    ...
    setClearApiKeyIds((current) => (willClear ? [...current, providerId] : current.filter((id) => id !== providerId)))
    if (willClear) {
    ...
    <div className="settings-split-view">
      <aside className="settings-list-panel">
        ...
        <button className="cliproxy-preset-button" onClick={addCliProxyPreset}>
          ...
          <span>
            <strong>{t('CLIProxyAPI local default')}</strong>
            <small>{t('127.0.0.1:8317 · Key optional')}</small>
          </span>
            ...
            >
              ...
              <span>
                ...
                <small>
                  {provider.hasApiKey
                    ? t('Key stored')
                    : isProviderKeyOptional(provider)
                      ? t('Local connection · Key optional')
                      : t('Requires API key')}
                </small>
              </span>
      ...
      {selectedProvider && (
        <div className="settings-editor provider-editor">
          <div className="editor-title-row">
            <div>
              <span className="entity-icon large provider-icon">
              ...
              <div>
                ...
                <small>
                  {selectedProvider.id === 'openrouter'
                    ? t('Built-in provider')
                    : selectedProvider.kind === 'cliproxy'
                      ? t('Local compatible proxy')
                      : t('Custom provider')}
                </small>
              </div>
            </div>
            <span className="settings-value-note">
              {clearApiKeyIds.includes(selectedProvider.id)
          ...
          <div className="editor-form-grid single-column">
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) => updateProvider({ kind: event.target.value as ProviderConfig['kind'] })}
              >
            ...
            <label>
              <FieldLabel hint={t('Responses API is recommended for new integrations')}>
                {t('Default API format')}
              </FieldLabel>
              <div className="provider-api-format-control">
                <select
                  aria-describedby="legacy-chat-completions-hint"
                  title={
                    selectedProvider.apiFormat === 'openai-chat-completions'
                      ? LEGACY_CHAT_COMPLETIONS_HINT
            ...
            <label>
              <FieldLabel
                hint={
                  selectedProvider.kind === 'cliproxy'
                    ? t('CLIProxyAPI default local listening address')
                    : t('Requests are sent to this URL')
                }
              >
                {t('Base URL')}
              </FieldLabel>
              <input
                ...
                placeholder={
                  selectedProvider.kind === 'cliproxy' ? 'http://127.0.0.1:8317/v1' : 'https://openrouter.ai/api/v1'
                }
            ...
            <label>
              <FieldLabel
                hint={
                  selectedProviderKeyOptional
                    ? t('If api-keys in config.yaml is empty, it can be left blank.')
                    : selectedProvider.hasApiKey
                      ? t('Saved encrypted; leave blank to remain unchanged')
                      : t('Encrypted with OS-protected storage when saved')
                }
              >
                {t('API key')}
              </FieldLabel>
              <div className="secret-input">
                ...
                <input
                  ...
                  placeholder={
                    clearApiKeyIds.includes(selectedProvider.id)
                      ? t('The key will be cleared when you save; enter a new value to keep a key instead')
                      : selectedProvider.hasApiKey
                        ? '••••••••••••••••••••'
                        : selectedProviderKeyOptional
                          ? t('Optional: Fill in the key configured by CLIProxyAPI')
                          : 'sk-or-v1-…'
                  }
              ...
              {selectedProviderNeedsNewKey && (
                <span className="credential-warning">
                  <Icon name="info" size={13} />
                  {t(
                    'The provider URL or type changed. For security, the saved API key will be cleared; enter it again.',
                  )}
                </span>
              )}
            </label>
          ...
          <div className="provider-security-banner">
            ...
            <div>
              <strong>
                {selectedProviderKeyOptional
                  ? t('Local loopback connections can be used without an API key')
                  : t('The key is never stored in persistent renderer state')}
              </strong>
              <p>
                {selectedProviderKeyOptional
                  ? t(
                      'If the api-keys of CLIProxyAPI are not configured, please keep it empty; it will still be securely encrypted when filled in.',
                    )
                  : t(
                      'When saved, the key is sent securely to the main process and encrypted using OS-protected storage.',
                    )}
              </p>
            </div>
          </div>
          {selectedProviderKeyOptional &&
            ...
            !(apiKeyInputs[selectedProvider.id] ?? '').trim() && (
              <div className="cliproxy-network-warning">
                ...
                <div>
                  <strong>{t('Restrict the server listen address when no key is configured')}</strong>
                  <p>
                    {t(
                      'In CLIProxyAPI config.yaml, set host: "127.0.0.1". The default host: "" may allow access from the local network, and TLS is disabled by default.',
                    )}
                  </p>
                </div>
              </div>
          ...
          <div className="provider-actions">
            ...
            {selectedProvider.id !== 'openrouter' && (
              <button className="remove-provider-button" onClick={removeProvider}>
