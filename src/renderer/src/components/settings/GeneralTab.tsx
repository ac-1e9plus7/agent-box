  ...
  return (
              <div className="settings-section-content narrow-settings">
                <section className="settings-card user-profile-card">
                  ...
                  <div className="user-profile-settings">
                    ...
                    <div className="user-profile-fields">
                      <label>
                        <FieldLabel hint={t("Maximum {value0} characters, can be left blank", { value0: MAX_USER_NICKNAME_LENGTH })}>{t("Nickname")}</FieldLabel>
                        <input
                  ...
                  <p className="user-profile-note">
                    <Icon name="shield" size={13} />{t("Your nickname and avatar are shown only in the local interface. They are never added to prompts or sent to a model.")}</p>
                  {avatarInputError && <p className="user-profile-error" role="alert">{avatarInputError}</p>}
                </section>
                <section className="settings-card">
                  <h3>{t("Appearance and behavior")}</h3>
                  <div className="settings-row">
                    <div><strong>{t("Display language")}</strong><small>{t("Change the language used by AgentBox and system dialogs")}</small></div>
                    <select
                  ...
                  <div className="settings-row">
                    <div><strong>{t("Theme")}</strong><small>{t("Use the system theme or choose a fixed theme")}</small></div>
                  ...
                  <div className="settings-row">
                    <div><strong>{t("Default Agent mode for new conversations")}</strong><small>{t("Agent mode and skill injection are enabled by default when creating a new conversation.")}</small></div>
                    <SettingsToggle
                  ...
                  <div className="settings-row">
                    <div><strong>{t("agent.toolCallTurns")}</strong><small>{t("Range: 1–100. Default: {value0}. Higher limits increase latency and API costs.", { value0: DEFAULT_AGENT_TOOL_TURN_LIMIT })}</small></div>
                    <AgentTurnLimitInput
                  ...
                  <div className="settings-row">
                    <div><strong>{t("Default reasoning for new conversations")}</strong><small>{t("Reasoning will be enabled automatically for models that support it")}</small></div>
                    <SettingsToggle
                  ...
                  <div className="settings-row">
                    <div><strong>{t("Default reasoning effort for new models")}</strong><small>{t("It is used as the default initial value when adding a model and can be adjusted individually on the model page.")}</small></div>
                    <select
                ...
                <section className="settings-card">
                  <h3>{t("Input")}</h3>
                  <div className="settings-row">
                    <div><strong>{t("Press Enter to send")}</strong><small>{t("Use ⌘/Ctrl + Enter to send after closing")}</small></div>
                    <SettingsToggle
                  ...
                  <div className="settings-row" style={{ marginTop: '16px' }}>
                    <div><strong>{t("Title generation model")}</strong><small>{t("Model used to generate conversation titles automatically")}</small></div>
                    <select
                ...
                <section className="settings-card">
                  ...
                  <div className="settings-row workspace-default-row">
                    <div>
                      ...
                      <small>{t("Shown as a shortcut in the New conversation dialog; you can still reuse an existing directory or choose another one")}</small>
                    </div>
                    <div className="workspace-default-actions">
                      <button
                        ...
                        type="button"
                      >
                        <Icon name="folder" size={14} />{t("Select directory")}</button>
                      {preferenceDraft.defaultWorkingDirectory && (
                        <button className="icon-button" aria-label={t("Clear default working directory")} onClick={() => setPreferenceDraft((current) => ({ ...current, defaultWorkingDirectory: '' }))}>
                  ...
                  <p className="workspace-default-path" title={preferenceDraft.defaultWorkingDirectory}>
                    {preferenceDraft.defaultWorkingDirectory || t("Not set; choose a working directory when creating a conversation")}
                  </p>
                </section>
                <section className="settings-card context-policy-card">
                  ...
                  <div className="context-policy-options">
                    ...
                    >
                      ...
                      <span>
                        <strong>{t("Manual management")}<em>{t("Default")}</em></strong>
                        <small>{t("Keep the full conversation history. Sending is blocked when it exceeds the model’s available context; shorten the conversation or adjust the context window.")}</small>
                      </span>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>{t("When the limit is exceeded, trim complete user–assistant turns starting with the oldest. Always keep the system prompt and latest question.")}</small>
                      </span>
                    </button>
                  </div>
                </section>
                <section className="settings-card context-policy-card">
                  ...
                  <div className="context-policy-options">
                    ...
                    >
                      <span className="policy-radio"><i /></span>
                      <span>
                        <strong>{t("Auto-select")}<em>{t("Recommended")}</em></strong>
                        <small>{t("Windows tries PowerShell 7, Windows PowerShell, cmd in order; macOS/Linux uses SHELL first, then zsh, bash, fish or sh.")}</small>
                      </span>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>{t("Use the executable file name or absolute path, and add startup parameters line by line.")}</small>
                      </span>
                    </button>
                  </div>
                  {preferenceDraft.integratedTerminalShell.mode === 'custom' && (
                    <div className="terminal-shell-fields">
                      <label className="system-prompt-field">
                        <FieldLabel hint={t("For example: pwsh.exe, C:\\Program Files\\PowerShell\\7\\pwsh.exe, /bin/zsh, /usr/bin/fish")}>{t("Shell executable")}</FieldLabel>
                        <input
                      ...
                      <label className="system-prompt-field">
                        <FieldLabel hint={t("Enter one argument per line. Recognized shells receive the appropriate command arguments automatically; for other shells, use the {command} placeholder.")}>{t("Startup parameters")}</FieldLabel>
                        <textarea
                  ...
                  <p className="settings-card-note">{t("The Agent uses this shell for integrated terminal calls. Under the default security policy, every command requires approval before execution.")}</p>
                ...
                <section className="settings-card context-policy-card">
                  ...
                  <div className="context-policy-options">
                    ...
                    >
                      <span className="policy-radio"><i /></span>
                      <span>
                        <strong>{t("Off")}<em>{t("Default")}</em></strong>
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>{t("Forward all model requests; http is available for local proxies and https for remote proxies.")}</small>
                      </span>
                    </button>
                  </div>
                  {preferenceDraft.proxy.mode === 'custom' && (
                    <label className="system-prompt-field">
                      <FieldLabel hint={t("Supports http:// (local machine only) and https://, username and password can be included in the address")}>{t("Proxy address")}</FieldLabel>
                      <input
                        className="mono-input"
