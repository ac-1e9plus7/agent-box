...
export function ModelsTab({
  ...
  const updateModel = (patch: Partial<ModelConfig>): void => {
    setModelDrafts((current) => current.map((model) => (model.id === selectedModelId ? { ...model, ...patch } : model)))
  }
        ...
        <aside className="settings-list-panel">
          <div className="settings-list-toolbar">
            ...
            <div>
              <button disabled={!onDiscoverModels || discovering} onClick={() => void discoverModels()}>
                {discovering ? <span className="button-spinner" /> : <Icon name="refresh" size={14} />}
                {t('Fetch')}
              </button>
          ...
          {remoteModels && (
            <div className="remote-model-picker">
              <header>
                <strong>{t('Select remote model')}</strong>
                <button aria-label={t('Close model list')} onClick={() => setRemoteModels(null)}>
              ...
              <div>
                {remoteModels.slice(0, 100).map((remoteModel) => (
                  <button key={remoteModel.id} onClick={() => addDiscoveredModel(remoteModel)}>
                    <span>
                      <strong>{remoteModel.name || remoteModel.id}</strong>
                      <small>{remoteModel.id}</small>
                    </span>
            ...
            {modelDrafts.map((model) => {
              ...
              return (
                ...
                >
                  ...
                  <span>
                    <strong>{model.name || t('Unnamed model')}</strong>
                    <small>{provider?.name ?? t('No provider selected')}</small>
                  </span>
                </button>
              )
            })}
        ...
        {selectedModel && (
          <div className="settings-editor">
            <div className="editor-title-row">
              <div>
                <span className="entity-icon large">
                  <Icon name="sparkles" size={18} />
                </span>
                <div>
                  <h3>{selectedModel.name || t('Unnamed model')}</h3>
                  <small>{selectedModel.remoteId || t('Fill in the model ID')}</small>
                </div>
              </div>
              ...
              <label>
                ...
                <select
                  value={
                    ...
                    providerDrafts.find((provider) => provider.id === selectedModel.providerId)?.apiFormat ??
            ...
            {modelsNeedingCalibration.includes(selectedModel.id) && (
              <div className="model-calibration-warning">
                ...
                <span>
                  <strong>{t('Remote interface does not return full model capabilities')}</strong>
                  <small>
                    {t(
                      'Missing capabilities use general defaults. Before saving, verify the context window, maximum output tokens, and reasoning support.',
                    )}
                  </small>
                </span>
                <button
                  onClick={() =>
                    setModelsNeedingCalibration((current) => current.filter((id) => id !== selectedModel.id))
                  }
                >
                  {t('Got it')}
                </button>
            ...
            {selectedModelProvider?.kind === 'openrouter' && (
              <section className="routing-card">
                <div className="routing-heading">
                  <span className="entity-icon provider-icon">
                ...
                <label className="routing-only-field">
                  <FieldLabel hint={t('Comma-separated; leave blank to let OpenRouter choose')}>
                    {t('Allowed provider slugs')}
                  </FieldLabel>
                  <input
                    className="mono-input"
                ...
                <div className="routing-toggles">
                  <div>
                    <span>
                      <strong>{t('Allow fallbacks')}</strong>
                      <small>{t('Switch to another provider when your preferred provider is unavailable')}</small>
                    </span>
                    <SettingsToggle
                      checked={selectedModel.providerRouting?.allowFallbacks ?? true}
                      label={t('Allow provider fallbacks')}
                      onChange={(allowFallbacks) =>
                        updateModel({
                  ...
                  <div>
                    <span>
                      <strong>{t('Zero Data Retention (ZDR) endpoints only')}</strong>
                      <small>{t('Require Zero Data Retention (ZDR) support')}</small>
                    </span>
                    <SettingsToggle
            ...
            {selectedModelApiFormat === 'anthropic-messages' && (
              <div className="anthropic-thinking-card">
                <div>
                  ...
                  <span>
                    <strong>{t('Anthropic thinking mode')}</strong>
                    <small>{t('Choose the mode supported by this Claude model')}</small>
                  </span>
                </div>
                <select
                  value={selectedModel.anthropicThinkingMode ?? 'adaptive'}
                  onChange={(event) =>
                    updateModel({ anthropicThinkingMode: event.target.value as 'adaptive' | 'manual' })
                  }
                >
                  <option value="adaptive">{t('Adaptive thinking (Claude 4.6+)')}</option>
                  <option value="manual">
                    {t('Manual extended thinking (Claude 4.5 and earlier; deprecated on 4.6)')}
                  </option>
                </select>
              </div>
            )}
            ...
            <div className="model-capability-card">
              <div>
                ...
                <span>
                  ...
                  <small>{t('Allow reasoning to be toggled for this model')}</small>
                </span>
              </div>
              <SettingsToggle
            ...
            <div className="model-capability-card nested-capability">
              <div>
                <span>
                  <strong>{t('Enable by default only for new conversations')}</strong>
                  <small>{t('Affects only newly created conversations; existing conversations are unchanged')}</small>
                </span>
              </div>
              <SettingsToggle
            ...
            <div className="model-capability-card nested-capability">
              <div>
                <span>
                  <strong>{t('Default reasoning effort')}</strong>
                  <small>{t('Reasoning effort used when reasoning is enabled for this model')}</small>
                </span>
              </div>
              <select
            ...
            <div className="danger-row">
              <button disabled={modelDrafts.length <= 1} onClick={removeModel}>
