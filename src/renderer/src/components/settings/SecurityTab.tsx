  ...
  return (
    <div className="settings-section-content narrow-settings">
      <div className="encryption-hero">
        ...
        <div>
          <h3>{t('Local data protection is enabled')}</h3>
          <p>{t('Conversations, configuration, and API keys are encrypted before being written to disk.')}</p>
        </div>
        <i>
      ...
      <section className="settings-card">
        <div className="settings-row">
          <div>
            ...
            <small>{t('Encrypted using OS credential protection')}</small>
          </div>
        ...
        <div className="settings-row">
          <div>
            <strong>{t('Conversation database')}</strong>
            <small>{t('Stored only in this device’s app data directory')}</small>
          </div>
          <span className="security-state">
            <Icon name="check" size={14} />
            {t('Local')}
        ...
        <div className="export-card-head">
          <div>
            ...
            <span>
              ...
              <small>
                {t('Export all conversations as plaintext JSON and Markdown, optionally including working directories')}
              </small>
            </span>
          </div>
          <button
          ...
          <div className="backup-export-panel">
            <fieldset className="backup-mode-options" disabled={backupExporting}>
              ...
              <label className={backupMode === 'shallow' ? 'is-selected' : ''}>
                ...
                />
                <span>
                  <strong>{t('Shallow backup')}</strong>
                  <small>
                    {t('All conversations, branches, attachments, and Agent records; excludes working directories')}
                  </small>
                </span>
              </label>
              <label className={backupMode === 'deep' ? 'is-selected' : ''}>
                ...
                />
                <span>
                  <strong>{t('Deep backup')}</strong>
                  <small>
                    {t(
                      'Includes everything in a shallow backup plus all unique conversation working directories recursively',
                    )}
                  </small>
                </span>
              </label>
            ...
            <div className={`backup-security-note ${backupProtectionEnabled ? 'is-protected' : 'is-warning'}`}>
              ...
              <span>
                <strong>
                  {backupResult
                    ? backupProtectionEnabled
                      ? t('This backup is protected with WinZip AES-256 (AE-2)')
                      : t('This backup is not password-protected')
                    : backupProtectionEnabled
                      ? t('File contents will be protected with WinZip AES-256 (AE-2)')
                      : t('An unencrypted plaintext ZIP will be exported')}
                </strong>
                <small>
                  {backupProtectionEnabled
                    ? t(
                        'AgentBox does not store passwords. ZIP entry names remain visible, and deep backups expose file names from working directories.',
                      )
                    : t(
                        'Conversations, attachments, and files in a deep backup can be read directly. Set a password before exporting.',
                      )}
                </small>
              </span>
            </div>
            ...
            {backupError && (
              <p className="backup-export-error" role="alert">
                {backupError}
              </p>
            )}
            {backupResult && (
              <div className="backup-export-success" role="status">
                ...
                <span>
                  ...
                  <small>
                    {t('{value0} conversations', { value0: backupResult.conversationCount })}
                    {backupResult.mode === 'deep'
                      ? t(', {value0} working directories', { value0: backupResult.workspaceCount })
                      : ''}
                    {backupResult.bytesWritten !== undefined
                      ? t(', {value0}', { value0: formatFileSize(backupResult.bytesWritten) })
                      : ''}
      ...
      <section className="settings-card danger-card">
        <div className="danger-card-head">
          ...
          <span>
            ...
            <small>
              {t(
                'Deletes all conversations and messages, then re-encrypts local data. Configured providers and models are kept.',
              )}
            </small>
          </span>
        </div>
        ...
        {clearConfirming ? (
          <div className="danger-card-confirm">
            ...
            <div className="danger-card-actions">
              ...
              <button className="danger-button" disabled={clearing} onClick={() => void confirmClearData()}>
                {clearing ? t('Clearing…') : t('Clear data')}
              </button>
            </div>
          </div>
        ) : (
          <button className="danger-button" disabled={!onClearData} onClick={() => setClearConfirming(true)}>
            <Icon name="trash" size={14} />
            {t('Clear all conversation data')}
          </button>
        )}
      </section>
    </div>
