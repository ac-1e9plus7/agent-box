...
export function createSkillLoaderTool(skills: Skill[]): McpToolDefinition | undefined {
  ...
  return {
    ...
    description: t("Load a local read-only Skill’s complete SKILL.md, reference documents, and reference scripts by Skill ID. Use it only when the active Skills are insufficient for the task. This tool never executes scripts."),
    inputSchema: {
      type: 'object',
      properties: {
        skill_id: {
...
export function createCodeRunnerTool(skills: Skill[]): McpToolDefinition | undefined {
  ...
  return {
    ...
    description: t("Run short, dependency-free code for algorithms or data validation. JavaScript runs in an isolated worker; Python runs only when Python 3 is installed locally. Execution can consume local system resources and usually requires user approval."),
    inputSchema: {
      type: 'object',
      properties: {
        language: {
          ...
          description: t("Prioritize using javascript to ensure availability; only choose python when the user clearly needs Python or the code must use Python."),
        },
...
export function createTerminalRunnerTool(): McpToolDefinition {
  return {
    ...
    description: t("Execute a command in the user's configured Integrated terminal shell. The shell is automatically selected by operating system, or the executable and startup parameters can be specified in settings. Terminal commands are sensitive operations and usually require user approval."),
    inputSchema: {
      type: 'object',
      properties: {
  ...
  return [
    {
      ...
      description: t("Reads a UTF-8 text file in the current conversation's working directory. path must be a path relative to the working directory; segmented reading by line is supported. Prioritize using this tool to read source code and configuration files to avoid escaping or truncation through shell output."),
      inputSchema: {
        type: 'object',
        properties: {
          path: { type: 'string', minLength: 1, maxLength: 4_096, description: t("File path relative to the current working directory.") },
          start_line: { type: 'integer', minimum: 1, description: t("Line number to start reading from, default is 1.") },
          max_lines: { type: 'integer', minimum: 1, maximum: 2_000, description: t("The maximum number of lines to read, the default is 400.") },
        },
        required: ['path'],
        additionalProperties: false,
      },
    ...
    },
    {
      ...
      description: t("Writes text directly to a file in the current conversation's working directory, without going through the shell, so code characters such as newlines, quotes, backticks, and $ are not interpreted twice. path must be a relative path; parent directories are automatically created. Read tool verification should be called as needed after writing."),
      inputSchema: {
        type: 'object',
        properties: {
          path: { type: 'string', minLength: 1, maxLength: 4_096, description: t("The target file path relative to the current working directory.") },
          content: { type: 'string', maxLength: 100_000, description: t("The complete text content to be written as UTF-8; for extremely long files, please split them and use append.") },
          mode: {
            type: 'string',
            enum: ['create', 'overwrite', 'append'],
            description: t("create creates a new file only; overwrite replaces or creates a file (default); append adds content to the end."),
          },
          create_parent_directories: { type: 'boolean', description: t("Create missing parent directories; defaults to true.") },
        },
