...
`web-extractor` Skill 会指导模型通过 `tab_id` 列出并跟踪标签页，先导航、再读取目标标签页经过明确批准的语义快照，只复用该标签页最新快照中的引用，每次交互后重新获取快照，交叉核对来源，并把网页指令和截图当作提示注入数据而不是 Agent 命令。只有匹配工具已经暴露且用户任务确实需要时才可截图、上传或下载。Skill 本身不会授予浏览能力：应用级和对话级开关仍控制工具暴露；所有网络、审批、工作区、大小和敏感字段限制仍由主进程强制执行。
