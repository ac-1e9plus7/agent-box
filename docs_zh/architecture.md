...
- **隔离式多标签浏览器**：可选内置浏览器由主进程中的 `BrowserManager` 持有。每个对话的 Session 初始化会合并并发请求，避免 renderer 竞态创建失管的原生 View；慢速初始化期间的显隐采用最后请求生效。新会话和用户新建的标签页都会导航到经过校验的 `AppSettings.browserHomePage`，其默认值为 Google。关闭最后一个标签页会关闭该对话的完整浏览器 Session，不再创建补位标签页。Agent 请求持有浏览器租约期间，用户关闭最后标签或整个 Session 的请求会延迟到请求清理阶段，关闭单独标签则会被阻止；Agent 发起的关闭工具以及应用生命周期清理仍立即执行。AgentBox 最多保留三个活动 Session，同时只显示一个 Session，每个 Session 最多十二个沙箱化 `WebContentsView` 标签页。只有当前可见的活动标签会附着到宿主窗口；隐藏或关闭会在 Cookie 持久化和网络清理异步收尾前，同步隐藏并卸载全部原生 View。打开第四个 Session 会关闭最久未使用的隐藏 Session。远程页面没有 preload、Node.js、AgentBox IPC、DevTools 或已授予的 Web 权限；新窗口会转换为经过策略检查的标签页。设置或新建对话框打开时会隐藏浏览器 View；删除对话、关闭窗口或退出应用时会销毁对应会话。Chromium Session 流量通过 `Session.setProxy` 使用自定义代理；供应商/MCP 的 `undici.ProxyAgent` 调度保持独立。下载、上传、截图、环回 HTTP 和 Cookie 持久化均使用互相独立的显式设置。
...
| 浏览器       | `browser:ensure`, `browser:navigate`, `browser:command`, `browser:new-tab`, `browser:switch-tab`, `browser:close-tab`, `browser:set-view-state`, `browser:close`, `browser:cancel-pending-close`, `browser:event` | 控制用户可见标签页和延迟 Session 关闭，并发布脱敏 `BrowserState` 元数据（`activeTabId`、标签 `id`）或下载名称/进度元数据；Cookie profile/值、浏览器绝对路径、DOM 状态、来源授权和活动页面存储仍留在主进程。经过批准的 Agent 快照和截图字节则作为 `chat:event` 工具结果单独传递，并成为加密对话数据。 |
