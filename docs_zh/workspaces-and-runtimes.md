...
- 全局“新建对话”面板可复用当前目录、设置中的默认目录或最近使用目录（最多显示 6 个），也可打开系统目录选择器。候选项按规范化完整路径去重。
...
- 使用真实路径去重，相同目录只归档一次，同时记录引用它的会话。
- 空目录会保留；符号链接以链接条目归档，不跟随其目标。
- 备份输出文件及其临时路径会从遍历中排除，避免把正在生成的归档再次打包。
...
候选 Shell 会通过 2 秒的空操作命令测试，并缓存第一个可用结果。指定模式接受可执行文件名或路径以及逐行启动参数。PowerShell、cmd、Fish 和常见 POSIX Shell 会自动添加各自的命令参数；其他自定义 Shell 可在启动参数中使用 `{command}` 占位符。
...
- 成功发现后，列表优先匹配当前配置，否则选择活动环境或首个环境，并保存实际 prefix 路径。发现失败时仍可手动填写环境名称或绝对 prefix。
