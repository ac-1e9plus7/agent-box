...
- 应用启动期间，主进程会确保 AgentBox 可执行文件旁存在 `.default-agent-box-workspace`。缺失时自动创建普通目录；若该路径已被文件或符号链接占用，则拒绝使用。设置中未指定其他默认值时，该应用同目录工作区就是默认工作目录。
- Vault 只保存规范化后的目录路径引用。项目文件不会因创建会话而被复制、封装或加密。
- 每次新建对话都会打开全局“新建对话”面板。置顶的推荐操作使用默认工作区；用户也可以切换到当前目录、最多 6 个仍被现有会话引用的最近工作目录，或打开系统目录选择器。设置中显式选择的默认目录优先于应用同目录的后备默认值。最近目录列表由保留的会话路径重建，并不是独立持久化的目录历史；候选项按规范化完整路径去重。
