...
- 编辑历史输入：可对任意一条自己发送过的消息进行编辑。编辑完成后可选择「仅保存」（只更新该条文本，保留其后所有对话）或「保存并重新生成」（更新文本后丢弃该条之后的所有内容，并用当前模型重新生成一条回复）。生成中的会话不允许编辑。
...
输入框右下角的 token 计数是与主进程请求时一致的估算：使用同一套 token 估算函数，并按主进程的注入规则计算系统提示词（会话历史不存储系统消息，配置的系统提示词每次请求注入一次，历史中已有相同内容时不重复计入）。
...
- **OpenRouter 上游供应商**：实际执行推理的 endpoint/provider。界面可配置 `only`、`allow_fallbacks`、排序、数据收集与 ZDR 等偏好，用于控制延迟、价格、数据策略或功能兼容性；是否真正可用仍取决于该模型当前的 OpenRouter 路由。新建模型与内置 OpenRouter Auto 默认采用**安全路由**：`data_collection: 'deny'`（拒绝留存用户数据的上游）+ `zdr: true`（仅使用零数据保留端点）；已有模型配置不会被覆盖。
...
- 思考文本统一从 OpenRouter 的 `reasoning_details`（`reasoning.text`/`reasoning.summary`，含 Gemini 的 `google-gemini-v1` 格式）、Responses 的 reasoning 事件，以及 Anthropic 的 thinking delta 中解析并展示。Gemini 经 OpenRouter 的思考 token（`thoughtsTokenCount`）会归一化到用量的 reasoning token 中显示。
...
在「设置 → 通用 → 网络代理」中可让所有模型请求（聊天流式请求与模型列表获取）经由自定义代理转发，便于在公司网络或受限环境中访问供应商。

- 默认关闭，直连所有供应商。
- 自定义模式支持 `http://` 与 `https://` 代理地址，可在地址中嵌入用户名密码（如 `https://user:pass@proxy.example.com:443`）。
- 出于安全考虑，远程 HTTP 代理不被允许：`http:` 仅限本机回环地址（如 `http://127.0.0.1:7890`），远程代理请使用 `https:`。
- 代理配置随 vault 本地加密保存；切换为「关闭」会保留已填地址，便于临时关闭后恢复。
- 代理仅作用于主进程发起的请求，不影响 renderer 与本地窗口行为；修改后下一次请求即生效。

### 清除会话数据

在「设置 → 数据与安全」中可「清除全部会话数据」：删除所有对话与消息，并重新加密落盘整份 vault。

- 只清除会话（聊天记录），**不会清除已配置的供应商、模型和 API Key**，清除后可直接用现有配置开启新对话。
- 执行时会先取消进行中的流式请求，再清空 vault 中的 `conversations` 并以新的随机 IV 重新加密写入；当前数据文件不再包含任何会话内容。
- 主密钥被保留（否则保留的供应商/模型配置将无法解密），因此历史上被替换覆盖的旧密文扇区虽不可经应用访问，但在理论上仍可被该主密钥解密。这是单文件加密 vault 在用户态下的固有限制；如需彻底销毁，请直接删除 `app.getPath('userData')/vault` 目录并重新配置。
- 该操作不可撤销。


> 代理服务器能看到你与供应商之间全部流量的目的地址与（HTTPS 下经 CONNECT 隧道转发的）密文。请只使用你信任的代理。

### 敏感数据安全加固

处理敏感内容时,可按以下方式最大化数据安全。新建模型已默认采用安全路由,其余为可选增强:

**默认已启用(新建模型/新 vault)**
- **数据收集策略 `deny`**:拒绝会留存用户数据(用于训练/改进)的 OpenRouter 上游 endpoint。
- **ZDR `true`**:仅使用声明零数据保留(Zero Data Retention)的 endpoint。ZDR 是 `deny` 的严格上位替代,二者叠加形成双保险。
- **联网搜索默认关闭**(`defaultWebSearchMode: 'off'`):避免把查询与上下文额外发送给搜索服务。
- **思考默认关闭**(`defaultReasoningEnabled: false`):减少不必要的推理 token 与费用。
- **远程强制 HTTPS**:非回环 Base URL 必须 `https:`,代理远程地址同样要求 HTTPS。
- **本地加密**:vault 用 AES-256-GCM 加密,密钥由系统凭据封装,无明文降级。

**可选增强(视场景手动开启)**
- **限定上游供应商**(`only`/`order`):在 OpenRouter 路由中用 `only` 指定官方/可信 endpoint slug(如 `anthropic`),避免回退到第三方上游。配合关闭「允许回退」可强制只走指定 endpoint,但会降低可用性。
- **手动清除会话数据**:处理完敏感对话后,在「设置 → 数据与安全」清除全部会话,重新加密落盘。注意旧密文扇区在用户态无法物理擦除(见上文「清除会话数据」),如需彻底销毁请删除 `vault` 目录。
- **系统级磁盘加密**:启用 BitLocker / FileVault / LUKS 等全盘加密,补足 vault 文件扇区残留这一层。
- **使用本机 CLIProxyAPI**:对最敏感的场景,可经本机回环 CLIProxyAPI 连接,避免数据直接出本机(取决于 CLIProxyAPI 的上游配置)。

**安全边界(这些设置不能做到的)**
- 消息必须以明文发给上游模型才能推理,本地加密与路由策略不构成端到端加密。
- `deny`/`zdr` 是发给 OpenRouter 的路由请求偏好,是否真正满足取决于 OpenRouter 当时的路由能力,客户端无法保证。
- 不防护已控制操作系统账户、进程内存或系统凭据的攻击者(见下文「威胁边界」)。
...
- 跨平台打包：仓库已配置 GitHub Actions 工作流（`.github/workflows/release.yml`），支持在发布标签（如 `v*`）时自动在 Windows、macOS 和 Linux 原生运行器上并行构建多平台安装包并创建 Release。
